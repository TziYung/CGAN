...
#%%create the embedding layer that would be applied in both generator and discriminator
def create_embedding():
    condition=keras.Input(1)
    layer=keras.layers.Embedding(10,128)(condition)
    return keras.Model(condition,layer)
...
def create_gen():
    ...
    layer=keras.layers.Dense(128)(embedding(condition))
...
def create_dis():
    ...
    condition=keras.Input((1))
    layer=keras.layers.Dense(784)(embedding(condition))
...
def plot_img(generator,noise,x):
        ...
        pyplot.savefig(f"D:\\mnist_gan_number_test\\{x}")
        pyplot.close('all')
        discriminator.save('discriminator_test.h5')
        generator.save('generator_test.h5')
